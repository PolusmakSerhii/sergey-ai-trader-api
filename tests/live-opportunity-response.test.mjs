...
for(const result of [{score:88,grade:'A+',confirmedAPlus:true},{score:86,grade:'A',confirmedAPlus:false}]){
 test(`live response preserves ${result.score}/${result.grade}/${result.confirmedAPlus} from one calculation`,()=>{
  assert.ok(start>0 && responseEnd>responseStart);
  let calls=0;const input={};const context={executionAnalysis:input,calculateScannerOpportunity:value=>{assert.equal(value,input);calls++;return result;}};
  for(const line of response.split('\n')){const name=line.trim().replace(/,$/,'');if(/^[a-zA-Z][a-zA-Z0-9]*$/.test(name))context[name]=null;}
  vm.runInNewContext(source.slice(start,end)+';globalThis.response={'+response+'};',context);
